...
def train():
    ...
    print("=" * 60)
    print("SCENARIO 2: Continuous MountainCar - Min Fuel Squared (TD3)")
    print("=" * 60)
    
    # Action noise for exploration
    action_noise = NormalActionNoise(
        mean=np.zeros(env.action_space.shape[0]),
        sigma=NOISE_SCALE * np.ones(env.action_space.shape[0]),
    )
    ...
    metrics_dir = ensure_dir("results/metrics/scenario2_td3")
    
    model.save(results_dir / "scenario2_td3")
